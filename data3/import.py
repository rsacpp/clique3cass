...
try:
    cur.execute("""
    insert into player0(id, clique, "globalId", pq, d, alias, "hashCode", setup) values(%s, '3', %s, %s, %s, %s, %s, now())
    """, [int(entryId), globalId, pq, "", alias, hashCode])
    conn.commit()
    ...
    os.popen("mv payer{0}.py {1}".format(alias, playerepo))
    os.popen("mv payer{0}    {1}".format(alias, playerepo))
